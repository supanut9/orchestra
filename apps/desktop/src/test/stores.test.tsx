...
describe('useSettingsStore', () => {
  ...
  it('sets and retrieves a provider config', async () => {
    ...
    expect((state.providers['anthropic'] as any).apiKey).toBe('sk-test-123');
  });
...
describe('useAgentStore', () => {
  ...
  it('sendMessage streams correctly via injected streamFn', async () => {
    ...
    const streamFn = vi.fn(async (_text: string, onChunk: (c: string) => void) => {
      onChunk('Hello ');
      onChunk('world');
    });

    await act(async () => {
      await useAgentStore.getState().sendMessage('test', streamFn as any);
    });
